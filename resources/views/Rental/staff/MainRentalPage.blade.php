...
<div class="bg-white rounded-xl shadow-xl border border-gray-100 p-6 md:p-8 w-11/12 mx-auto -mt-16 relative">
    ...
    <div class="flex flex-wrap justify-end items-center gap-4 mb-8 pb-6 border-b border-gray-200">
        <a href="{{ route('staff.rentals.current') }}" 
           class="bg-zinc-700 text-white font-semibold px-4 py-2 rounded-lg shadow hover:bg-zinc-800 transition transform hover:scale-105">
           <i class="bi bi-box-arrow-up-right mr-2"></i> View Current Rent
        </a>

        <a href="{{ route('staff.rentals.returnApproval') }}" 
           class="bg-yellow-500 text-white font-semibold px-4 py-2 rounded-lg shadow hover:bg-yellow-600 transition transform hover:scale-105">
           <i class="bi bi-clock-history mr-2"></i> Pending Returns
        </a>

        <a href="{{ route('staff.rental.add') }}" 
           class="bg-lime-600 text-white font-semibold px-4 py-2 rounded-lg shadow hover:bg-lime-700 transition transform hover:scale-105">
    ...
    <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($unavailableItems as $item)
                <div class="bg-gray-100 rounded-xl shadow-md p-6 hover:shadow-lg transition">
                    <h4 class="text-lg font-bold text-gray-800">{{ strtoupper($item->item_Name) }}</h4>
                    <p class="text-gray-600 mt-2">{{ strtoupper($item->item_Description) }}</p>
                    <div class="flex justify-between items-center mt-4">
                        <span class="font-semibold text-gray-700">Qty: {{ strtoupper($item->item_Quantity) }}</span>
    ...
    button.addEventListener('click', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // We use the new route with the CSRF token
                ...
                .catch(error => {
                    ...
                    Swal.fire(
                        'Error!',
                        'Failed to delete the item. It might be associated with active rentals.',
                        'error'
                    );
                });
            }
