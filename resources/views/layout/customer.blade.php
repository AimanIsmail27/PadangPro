...
<head>
    ...
    @php
        // Pull customer avatar from DB (same field your profile page uses)
        // Note: This is safe and works immediately, but does 1 DB query per page load.
        // Best practice later: store this path in session after upload/login.
        $customerAvatar = null;

        try {
            if (session()->has('user_id')) {
                $customerAvatar = \App\Models\Customer::where('userID', session('user_id'))->value('customer_Image');
            }
        } catch (\Throwable $e) {
            $customerAvatar = null;
        }
    ...
    <aside id="sidebar"
        ...
        <div class="flex items-center justify-between mb-10 px-2">
            <div class="flex items-center space-x-3">
                <img src="{{ asset('images/logoPadang.png') }}" alt="Logo"
                     class="w-9 h-9 object-contain rounded-full bg-white p-1">
            ...
            <ul class="space-y-2">
                ...
                <li class="nav-item">
                    <a href="{{ route('customer.dashboard') }}"
                       class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-700 transition-all {{ (request()->routeIs('customer.dashboard')) ? 'active' : '' }}">
                ...
                <li class="nav-item">
                    <a href="{{ route('customer.profile') }}"
                       class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-700 transition-all {{ (request()->routeIs('customer.profile')) ? 'active' : '' }}">
                ...
                <li class="nav-item" x-data="{ open: {{ request()->routeIs('booking.*') ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                            class="w-full sidebar-link flex items-center justify-between px-4 py-3 rounded-lg hover:bg-slate-700 transition-all {{ (request()->routeIs('booking.*')) ? 'active' : '' }}">
                ...
                {{-- Rental --}}
                <li class="nav-item">
                    <a href="{{ route('customer.rental.main') }}"
                       class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-700 transition-all {{ (request()->routeIs('customer.rental.*')) ? 'active' : '' }}">
                ...
                <li class="nav-item" x-data="{ open: {{ request()->routeIs('matchmaking.*') ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                            class="w-full sidebar-link flex items-center justify-between px-4 py-3 rounded-lg hover:bg-slate-700 transition-all {{ (request()->routeIs('matchmaking.*')) ? 'active' : '' }}">
                        <div class="flex items-center gap-3">
                ...
                <li class="nav-item">
                    <a href="{{ route('customer.rating.main') }}"
                       class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-700 transition-all {{ (request()->routeIs('customer.rating.*')) ? 'active' : '' }}">
        ...
        <div class="mt-auto pt-6 border-t border-slate-700">
            <a href="{{ route('logout') }}"
               class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg text-slate-400 hover:bg-red-600 hover:text-white transition-all">
    ...
    <div class="flex-1 flex flex-col min-h-screen transition-all duration-300 w-full lg:ml-64">
        ...
        <header class="bg-white shadow-sm p-4 sticky top-0 z-20 border-b border-gray-200 flex justify-between items-center">
            ...
            <div class="flex items-center gap-3 ml-auto">
                ...
                <div class="w-10 h-10 rounded-full overflow-hidden border border-indigo-200 bg-indigo-100 flex items-center justify-center font-bold">
                    @if($customerAvatar)
                        <img src="{{ asset('storage/' . $customerAvatar) }}" alt="Profile Photo" class="w-full h-full object-cover">
                    @else
                        <span class="text-indigo-600 uppercase">
                            {{ substr($fullName ?? session('full_name', 'C'), 0, 1) }}
                        </span>
                    @endif
                </div>
            </div>
