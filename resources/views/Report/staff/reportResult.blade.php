...
<div class="bg-gradient-to-r from-lime-500 via-emerald-500 to-lime-400 text-white pt-8 pb-24 px-6 sm:px-10 rounded-lg shadow-lg">
...
<div class="bg-white rounded-xl shadow-md border border-gray-100 p-5 sm:p-8 w-[95%] sm:w-11/12 mx-auto -mt-16 relative overflow-hidden">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        ...
        <button id="downloadPdfBtn"
                class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 px-4 rounded-lg shadow-md transition w-full sm:w-auto">
            Download PDF
        </button>
        ...
        @if(!$request->has('view_only'))
                ...
                <a href="{{ route('staff.reports.create', $request->query()) }}"
                   class="text-sm text-emerald-700 hover:text-emerald-900 font-semibold text-center w-full sm:w-auto">
                    Edit Criteria
                </a>
            ...
            @else
                <a href="{{ route('staff.reports.published') }}"
                   class="text-sm text-gray-600 hover:text-gray-900 text-center w-full sm:w-auto">
    ...
    @else
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            ...
            {{-- TOTAL --}}
            <div class="rounded-xl border bg-lime-50/60 p-4 overflow-hidden">
                <p class="text-xs text-lime-700 font-semibold uppercase tracking-wide">Total</p>
                <p class="text-2xl font-extrabold text-lime-900 mt-1 break-words">
                    {{ $summaryData['total'] ?? '—' }}
                </p>
                <p class="text-xs text-lime-700/80 mt-1">
...
{{-- PUBLISH FORM --}}
@if(!$request->has('view_only'))
<form id="publishForm" action="{{ route('staff.reports.publish') }}" method="POST" class="hidden">
    @csrf
...
{{-- PDF DOWNLOAD FORM --}}
<form id="pdfForm" action="{{ route('staff.reports.pdf') }}" method="POST" class="hidden">
    @csrf
