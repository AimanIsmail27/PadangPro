...
@section('content')
<style>
    /* These styles create the identical look of the customer page */
    .main { padding: 30px; }
    .profile-header { color: white; font-size: 20px; font-weight: bold; padding: 20px 30px; }
    .profile-card { background: white; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); padding: 30px; width: 80%; margin: 0 auto; position: relative; top: -40px; }
    ...
    .save-btn { background: #007bff; color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; font-size: 16px; }
    .cancel-btn { background: #6c757d; color: white; border: none; padding: 10px 20px; border-radius: 6px; text-decoration: none; display: inline-block; margin-left: 10px; font-size: 16px; }
</style>

<div class="main">
    <div class="bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-300 text-white pt-8 pb-24 px-10 rounded-lg shadow-lg">
        <h1 class="text-2xl font-bold">ADMIN BOOKING FORM</h1>
    </div>
    ...
    <div class="profile-card">
        <div class="mb-6">
            ...
            <p><strong>Time:</strong> {{ \Carbon\Carbon::parse($slot->slot_Time)->format('h:i A') }}</p>
        ...
        <form action="{{ route('admin.booking.store') }}" method="POST">
            ...
            {{-- THE INCORRECT HIDDEN userID FIELD IS NOW REMOVED --}}
            ...
            <div class="form-group">
                <label for="booking_BackupNumber">Backup Phone Number (Optional)</label>
                <input type="text" name="booking_BackupNumber" id="booking_BackupNumber">
            </div>

            <button type="submit" class="save-btn">Confirm Reservation</button>
            <a href="{{ route('admin.booking.manage') }}" class="cancel-btn">Cancel</a>
        </form>
    </div>
</div>
@endsection