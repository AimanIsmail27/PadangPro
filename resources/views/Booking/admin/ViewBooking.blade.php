...
<div class="bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-300 text-white pt-8 pb-24 px-10 rounded-lg shadow-2xl">
    <h1 class="text-3xl font-bold">Manage All Bookings</h1>
    <p class="mt-2 text-amber-100">Review, edit, or cancel staff and customer bookings.</p>
</div>
...
<div class="bg-white rounded-xl shadow-xl border border-gray-100 p-6 md:p-8 w-11/12 mx-auto -mt-16 relative space-y-12">
    ...
    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
        <form action="{{ route('admin.booking.viewAll') }}" method="GET" class="flex flex-wrap items-center gap-4">
            
            {{-- Month Filter --}}
            <div>
                <label for="month" class="text-sm font-medium text-gray-700">Filter by Month:</label>
                <select name="month" id="month" onchange="this.form.submit()" class="block w-52 rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm mt-1">
            ...
            <div>
                ...
                <input type="date" name="search_date" id="search_date" value="{{ request('search_date') }}"
                       class="block w-52 rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm mt-1">
            </div>

            {{-- Status Filter --}}
            <div>
                <label for="status" class="text-sm font-medium text-gray-700">Filter by Status:</label>
                <select name="status" id="status" onchange="this.form.submit()" class="block w-52 rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm mt-1">
            ...
            <div class="mt-6">
                ...
                <a href="{{ route('admin.booking.viewAll') }}" class="text-sm text-gray-600 hover:text-gray-900 ml-3">Clear Filters</a>
            </div>
    ...
    <div>
                ...
                <div class="overflow-x-auto shadow-md rounded-xl border border-gray-100">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-zinc-800">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-amber-300 uppercase tracking-wider">Title / Customer Name</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-amber-300 uppercase tracking-wider">Reserved By</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-amber-300 uppercase tracking-wider">Field</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-amber-300 uppercase tracking-wider">Date & Time</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-amber-300 uppercase tracking-wider">Status</th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-amber-300 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($adminBookings as $booking)
                                <tr class="hover:bg-gray-50/50 transition-colors {{ $booking->isExpired ? 'opacity-60' : '' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        @if(!$booking->isExpired && $booking->booking_Status !== 'completed')
                                            <div class="flex justify-center items-center gap-2">
                                                {{-- Record Cash --}}
                                                <form action="{{ route('admin.booking.payment.markCompleted', $booking->bookingID) }}" method="POST">
                                                    @csrf
                                                    <button type="button" class="record-cash-btn inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-green-600 text-white hover:bg-green-700 transition shadow">
                                                        <i class="bi bi-cash-coin mr-1"></i> Record Cash
                                                    </button>
                                                </form>

                                                {{-- Edit --}}
                                                <a href="{{ route('admin.booking.edit', $booking->bookingID) }}"
                                                class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-blue-600 text-white hover:bg-blue-700 transition shadow">
                                                ...
                                                <form action="{{ route('admin.booking.cancel', $booking->bookingID) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="cancel-booking-btn inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-red-600 text-white hover:bg-red-700 transition shadow">
        ...
        <div class="md:hidden space-y-4">
            @forelse($adminBookings as $booking)
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm {{ $booking->isExpired ? 'opacity-60' : '' }}">
                    ...
                    @if(!$booking->isExpired && $booking->booking_Status !== 'completed')
                        <div class="mt-3 space-y-2">
                            ...
                            <form action="{{ route('admin.booking.payment.markCompleted', $booking->bookingID) }}" method="POST">
                                @csrf
                                <button type="button" class="record-cash-btn w-full inline-flex justify-center items-center px-4 py-2 rounded-lg text-sm font-semibold bg-green-600 text-white hover:bg-green-700 transition shadow">
                            ...
                            {{-- Edit --}}
                            <a href="{{ route('admin.booking.edit', $booking->bookingID) }}"
                               class="w-full inline-flex justify-center items-center px-4 py-2 rounded-lg text-sm font-semibold bg-blue-600 text-white hover:bg-blue-700 transition shadow">
                            ...
                            <form action="{{ route('admin.booking.cancel', $booking->bookingID) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="cancel-booking-btn w-full inline-flex justify-center items-center px-4 py-2 rounded-lg text-sm font-semibold bg-red-600 text-white hover:bg-red-700 transition shadow">
            ...
            @else
                <div class="overflow-x-auto shadow-md rounded-xl border border-gray-100">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-zinc-800">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-amber-300 uppercase tracking-wider">Booked By</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-amber-300 uppercase tracking-wider">Field</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-amber-300 uppercase tracking-wider">Date & Time</th>
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        @if($booking->display_Status === 'Awaiting Balance')
                                            <form action="{{ route('admin.booking.payment.markCompleted', $booking->bookingID) }}" method="POST" class="inline-block">
                                                @csrf
        ...
        <div class="md:hidden space-y-4">
            @forelse($customerBookings as $booking)
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <div class="mt-2">
                        @if($booking->display_Status === 'Awaiting Balance')
                            <form action="{{ route('admin.booking.payment.markCompleted', $booking->bookingID) }}" method="POST" class="inline-block w-full">
                                @csrf
