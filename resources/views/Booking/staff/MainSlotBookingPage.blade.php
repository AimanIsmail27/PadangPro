...
<div class="container mx-auto px-4 pb-12">
    ...
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center">
        <div>
            <h2 class="text-3xl font-bold text-gray-800">Booking for Field: {{ $field->field_Label ?? 'Unknown' }}</h2>
            <p class="text-gray-500 mt-1">Select a time slot to book for yourself or your client.</p>
        </div>
    </div>
    ...
    <div class="mt-12" id="reviews-section">
        <h3 class="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
            <i class="bi bi-chat-quote-fill text-indigo-600"></i> Field Reviews
        </h3>

        @if($reviews->isEmpty())
            <div class="bg-gray-50 rounded-xl p-8 text-center border border-dashed border-gray-300">
                <p class="text-gray-500 italic">No reviews yet for this field.</p>
            </div>
        @else
...
.fc-button-primary { background-color: #4f46e5 !important; border-color: #84cc16 !important; }
...
<div id="landscapeHint" class="fixed bottom-0 left-0 right-0 p-3 z-50 shadow-2xl bg-lime-600/95 backdrop-blur-sm">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (calendarEl) {
            ...
            eventClick: function(info) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed && slotStatus === 'available') {
                        window.location.href = `/staff/booking/${slotId}/add`;
                    }
                });
            }
        });
        ...
        window.addEventListener('resize', function() {
            ...
            let currentHeaderState = calendar.view.type === 'timeGridDay';
